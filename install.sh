...
# On a fresh install, with no --sub=/--vless=, this drops straight into
# the interactive setup wizard (primary link, backup link, SOCKS/HTTP
# ports, Proxy0) -- it reads from /dev/tty rather than this script's own
# stdin, so it works even piped through `curl | sh` like the command
# above. Re-running this on an already-configured router (an update,
# including the bot's own "🔁 Обновить агент" button) skips setup
# entirely and keeps the existing config, --sub=/--vless= or not.
#
