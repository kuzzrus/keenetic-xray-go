...
| `cmd/geo-gen` + `presets/` | CI-only (not shipped in the router binary). `geo-gen` rebuilds `presets/*.lst` (curated per-service domain and IPv4-CIDR lists) from the upstreams in `presets/sources.json` -- Geo-Aggregator for domains, `lord-alfred/ipranges` (+ Telegram's own list) for CIDR -- validating every line through `config.ClassifyRouteEntry`, aggregating CIDR blocks, capping, and holding the previous file when a fetch drifts too far. `.github/workflows/geo-update.yml` runs it daily and commits the refresh to `main`. A later change wires the bot/CLI to pull routing lists from these |
