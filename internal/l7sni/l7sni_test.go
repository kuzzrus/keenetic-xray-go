...
func buildIPv4TCP(src, dst [4]byte, sport, dport uint16, payload []byte) []byte {
	ipHdr := make([]byte, 20)
...
func TestParseIPv4_TCP(t *testing.T) {
	...
	pkt := buildIPv4TCP(src, dst, 54321, 443, []byte("hello"))

	proto, gotSrc, gotDst, sport, dport, payload, ok := ParseIPv4(pkt)
	if !ok {
...
func TestParseIPv4_UDP(t *testing.T) {
	...
	proto, _, _, sport, dport, payload, ok := ParseIPv4(pkt)
...
func TestParseIPv4_RejectsUnknownProto(t *testing.T) {
	...
	if _, _, _, _, _, _, ok := ParseIPv4(ipHdr); ok {
...
func TestParseIPv4_RejectsShortTCPHeader(t *testing.T) {
	src := [4]byte{1, 2, 3, 4}
	dst := [4]byte{5, 6, 7, 8}
	pkt := buildIPv4TCP(src, dst, 1, 2, nil)
